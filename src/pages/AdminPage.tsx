...
export default function AdminPage() {
  ...
  const fetchTests = () => axios.get(`${url}/tests/`, axiosConfig).then((res) => setTests(res.data));
  const fetchBusinesses = () => axios.get(`${url}/businesses/`, axiosConfig).then((res) => setBusinesses(res.data));

  const handleLogin = () => {
    if (username === "1" && password === "1") {
      setIsLoggedIn(true);
      setError("");
    } else setError("Username yoki password noto‘g‘ri!");
  };
  ...
  const handleDeleteTest = (id: number) => {
    if (window.confirm("Siz rostdan ham ushbu testni o‘chirmoqchimisiz?")) {
      axios.delete(`${url}/tests/${id}`).then(() => setTests(tests.filter((t) => t.id !== id)));
    }
  };
  ...
  const handleAddBusiness = () => {
    ...
    axios.post(`${url}/businesses/`, biz).then((res) => {
      ...
      setNewBusiness(""); setShowBusinessModal(false);
    });
  };
  ...
  if (!isLoggedIn) {
    return (
      <div className="min-h-screen flex justify-center items-center bg-gradient-to-br from-purple-500 to-indigo-500">
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 120 }} className="bg-white p-8 rounded-3xl shadow-2xl w-[400px]">
          <h1 className="text-2xl font-bold text-indigo-600 mb-6 text-center">🔑 Admin Login</h1>
          {error && <p className="text-red-500 mb-4 text-center">{error}</p>}
          <input type="text" placeholder="Username" value={username} onChange={(e) => setUsername(e.target.value)} className="border w-full p-3 rounded-lg mb-3 focus:ring-2 focus:ring-indigo-400 outline-none"/>
          <input type="password" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} className="border w-full p-3 rounded-lg mb-4 focus:ring-2 focus:ring-indigo-400 outline-none"/>
          <button onClick={handleLogin} className="bg-indigo-600 text-white px-4 py-2 rounded-lg w-full hover:bg-indigo-700 transition">✅ Login</button>
        </motion.div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-500 to-purple-600 p-8">
      <h1 className="text-3xl font-bold text-white mb-6 text-center drop-shadow-lg">📋 Admin Panel — Testlar va Bizneslar</h1>

      <div className="max-w-6xl mx-auto">
        <div className="mb-12">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-semibold text-white">📝 Testlar</h2>
            <button onClick={() => setShowTestModal(true)} className="bg-white text-indigo-600 font-semibold px-5 py-2 rounded-xl shadow-lg hover:bg-indigo-100 transition">➕ Yangi Test Qo‘shish</button>
          </div>
        ...
        <div>
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => setShowBusinessModal(true)} className="bg-white text-indigo-600 font-semibold px-5 py-2 rounded-xl shadow-lg hover:bg-indigo-100 transition">➕ Yangi Biznes Qo‘shish</button>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            <AnimatePresence>
              {businesses.map((biz) => (
                <motion.div key={biz.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }} transition={{ duration: 0.3 }} className="bg-white rounded-2xl shadow-md hover:shadow-xl transition p-6 border border-gray-200 flex justify-between items-center">
                  <p className="text-xl font-bold text-gray-800">{biz.types}</p>
                  <button onClick={() => handleDeleteBusiness(biz.id)} className="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600 transition">🗑 O‘chirish</button>
                </motion.div>
              ))}
            </AnimatePresence>
          </div>
      ...
      <AnimatePresence>
        {showTestModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} transition={{ type: "spring", stiffness: 120 }} className="bg-white rounded-2xl shadow-2xl w-[400px] p-6">
              ...
              <div className="flex justify-end gap-3 mt-6">
                <button onClick={() => setShowTestModal(false)} className="bg-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-400 transition">❌ Bekor qilish</button>
                <button onClick={handleAddTest} className="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition">✅ Saqlash</button>
              </div>
            </motion.div>
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {showBusinessModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} transition={{ type: "spring", stiffness: 120 }} className="bg-white rounded-2xl shadow-2xl w-[400px] p-6">
              <h2 className="text-xl font-bold text-indigo-600 mb-4 text-center">➕ Yangi Biznes Qo‘shish</h2>
              <input type="text" placeholder="Biznes nomi" value={newBusiness} onChange={(e) => setNewBusiness(e.target.value)} className="border w-full p-3 rounded-lg focus:ring-2 focus:ring-indigo-400 outline-none mb-4"/>
              <div className="flex justify-end gap-3 mt-2">
                <button onClick={() => setShowBusinessModal(false)} className="bg-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-400 transition">❌ Bekor qilish</button>
                <button onClick={handleAddBusiness} className="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition">✅ Saqlash</button>
              </div>
            </motion.div>
          </motion.div>
        )}
